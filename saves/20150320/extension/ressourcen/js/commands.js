	...
	this.initview = function(view,params){
		...
		window.setTimeout(function(){
			if (cur_view.hasOwnProperty('permission')){
				permissonsallowed = checkpermission(permid,cur_view.permission);
			}
			if (cur_view.hasOwnProperty('origin')){
				originsallowed = checkorigin(permid,cur_view.origin);
			}
		},300);			
...
function commands(){
	...
	this.weather = function(){
		...
		this.getweather = function(view,urlplus){			
			$.ajax({
				url: "http://api.openweathermap.org/data/2.5/weather?" + urlplus + "&units="+localStorage["units"]+"&lang="+hl,
				dataType: 'JSON',
		...
		this.getview = function(view,params){
			view.html(getmsg("weather in")+' '+params.join(" "));
	...
	this.info = function(){
		...
		this.getview = function(view,params){
			if(params[0] == 'app' || params[0] == 'developer' || params[0] == 'support'){
				...
				cur_obj['info'].speak(view,getmsg("someinfosabout")+params[0]);	
			}	
	...
	this.notes = function(){
		...
		this.shownotes = function(view){
			...
			var hresults = new ObjResults();
			if(typeof(results) != 'undefined') if(view.attr('id') == results.attr('id')) hresults.addrows(getmsg("typetoadd"),'');
			...
			switch(params[0]){
				case 'add':
					...
					if (params.join(" ").length > 3){
						...
						view.html(getmsg("addedtonotes").replace('{note}',params.join(" ")));
					}
					else{
						view.html(getmsg("enteryournote"));
					}
				break;
				default:
					cur_obj['notes'].shownotes(view);
	...
	this.alarm = function(){
		...
		this.getallalarms = function(view){
			var hresults = new ObjResults();
			view.html(getmsg("typeadd"));
			chrome.alarms.getAll(function(data){
				var count = 0;
				data.forEach(function(data2){
					hresults.addrows(getmsg("alarmringin").replace('{min}',Math.round((data2.scheduledTime-Date.now())/10/60)/100));
			...
			switch(params[0]){
				case 'add':
					if(typeof(params[1]) != 'undefined' && params[1].length > 0 && !isNaN(params[1])){
						...
						view.html(getmsg("alarmringin").replace('{min}',params[1]));
						cur_obj['alarm'].speak(view,getmsg("alarmringin").replace('{min}',params[1]));
					}
					else{
						view.html(getmsg("enteratime"));
					}
				break;
				default:
					cur_obj['alarm'].getallalarms(view);
