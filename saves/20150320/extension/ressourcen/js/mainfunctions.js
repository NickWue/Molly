...
function loadcards(container){
	...
	for (cardscount = 0; cardscount <= applyedcards.length-1; cardscount++ ){
		if (applyedcards[cardscount].length > 2){
			...
			id = generateUUID();
			container.append('<div class="card '+id+'"><div class="headline"><span class="caption">'+applyedcards[cardscount]+'</span><span class="close"> x </span></div><div class="content"></div></div>')
	...
	ajaxreadycontainer = container;
	$(document).ajaxComplete(function() {
		$(ajaxreadycontainer).masonry();
	});
}
